...
with tab1:
    ...
    if 'last_aid' in st.session_state:
        ...
        if st.button("Selesai (Reset)"): del st.session_state['last_aid']; st.rerun()
            ...
            for _, row in df.iterrows():
                with st.expander(f"📦 {row['asset_id']} - {row['asset_name']}"):
                    ...
                    if c_b.button("Moving", key=f"mov_{row['asset_id']}"): 
                        st.session_state['act'] = 'mov'; st.session_state['m_id'] = row['asset_id']
                    if c_c.button("Delete", key=f"del_{row['asset_id']}"): st.warning("Fitur hapus diaktifkan")
    
    if st.session_state.get('act') == 'mov':
        st.divider()
        st.subheader(f"🚚 Moving Asset: {st.session_state['m_id']}")
        with st.form("mov_form"):
            t_no = st.text_input("No. Transmittal")
            conn = sqlite3.connect('assets.db')
            all_items = pd.read_sql("SELECT asset_id FROM assets", conn)['asset_id'].tolist()
            conn.close()
            selected = st.multiselect("Tambah item lain", options=all_items, default=[st.session_state['m_id']])
            if st.form_submit_button("Submit Pindahan"):
                st.success(f"Berhasil pindah! Items: {selected} | Transmittal: {t_no}")
