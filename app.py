...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS assets (
        asset_id TEXT PRIMARY KEY, asset_name TEXT, brand TEXT, model_type TEXT, serial_number TEXT, 
        category TEXT, sub_category TEXT, item_type TEXT, qty TEXT, uom TEXT, condition TEXT, 
        current_status TEXT, current_project TEXT, current_area TEXT, current_location TEXT, 
        storage_type TEXT, cabinet_rack TEXT, shelf TEXT, bin TEXT, purchase_date TEXT, 
        supplier TEXT, po_number TEXT, purchase_price TEXT, remark TEXT, image_filename TEXT)''')
...
with tab1:
    ...
    if st.session_state.get('show_input'):
        with st.form("input_form"):
            st.subheader("Input Data (25 Kolom)")
            cols = st.columns(2)
            # Contoh kolom (silakan lengkapi sisa kolom sesuai 25 daftar kamu)
            aid = cols[0].text_input("Asset ID")
            name = cols[1].text_input("Asset Name")
            pdate = cols[0].date_input("Purchase Date").strftime("%Y-%m-%d")
...
with tab3:
    ...
    if uploaded_file:
        ...
        conn.close(); st.success("Data Excel berhasil diunggah!")