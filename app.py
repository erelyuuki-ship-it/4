...
with tab1:
    with st.form("input_form", clear_on_submit=False): # Set False agar data tetap di layar setelah simpan
        c1, c2 = st.columns(2)
        with c1:
        ...
        if st.form_submit_button("Simpan Data"):
            ...
            st.image(buf, caption="QR Code Asset")
            st.download_button("Print/Download QR", data=buf.getvalue(), file_name=f"QR_{aid}.png", mime="image/png")

with tab2:
    # Scan via file upload juga bisa, tapi untuk kamera kita pakai kamera khusus scan
